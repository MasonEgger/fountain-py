...
class FountainParser:
    ...
    def parse(self, text: str) -> FountainDocument:
        ...
        # recovery below clears it, so parse() surfaces them on the returned document
        # (and validate() reads them from there).
        if self.in_boneyard:
        ...
        return FountainDocument(self.elements, metadata, issues=list(self.diagnostics))
    ...
    def validate(self, text: str) -> list[ValidationIssue]:
        ...
        Examples:
            ...
            'error'
        """
        # parse() records every diagnostic it can detect on the returned document, so
        # validation is just a parse whose issues are surfaced as the return value.
        return list(self.parse(text).issues)
    ...
    def _parse_line(
        ...
        if (
            ...
            # following is demoted to action below. Flag it as a diagnostic: a writer who
            # meant it as a cue has an orphaned character with no lines. The diagnostic is
            # surfaced on the parsed document and via validate().
            self.diagnostics.append(
                ValidationIssue(
                    line_number=self.current_line + 1,
                    severity="warning",
    ...
    def _is_dialogue_line(self, had_blank_line_before: bool = False) -> bool:
        ...
        # A lyric, standalone note, or boneyard comment inside a dialogue block does not end
        # the block: a character who sings (or carries an editorial note or a commented-out
        # line) mid-speech and then keeps speaking stays in dialogue. Look back past those
        # non-terminating lines to the element that anchors the block; a blank line still
        # ends it (had_blank_line_before).
        non_terminating = (ElementType.LYRICS, ElementType.NOTE, ElementType.BONEYARD)
        if prev_element.type in non_terminating and not had_blank_line_before:
            for earlier in reversed(self.elements):
                if earlier.type in non_terminating:
                    continue
